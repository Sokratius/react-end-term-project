...
const Signup: React.FC = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!validatePassword(password)) {
      setError('Password must be at least 8 chars, with 1 special char and 1 number.');
      return;
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-[80vh]">
      <div className="w-full max-w-md bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
        <h2 className="text-3xl font-bold text-white mb-6 text-center">{t('signup')}</h2>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
           ...
           <div>
            <label className="block text-sm font-medium text-slate-400 mb-1">{t('confirm_password')}</label>
            <input
              type="password"
